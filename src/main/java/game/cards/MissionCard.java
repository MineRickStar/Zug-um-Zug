...
public class MissionCard implements Comparable<MissionCard> {
	...
	public boolean equals(Object obj) {
		...
		return (this.distance == other.distance) && Objects.equals(this.getFromLocation(), other.getFromLocation())
				&& (this.points == other.points) && Objects.equals(this.getToLocation(), other.getToLocation());
	}

	@Override
	public String toString() {
		return System.lineSeparator() + "Card [fromLocation=" + this.getFromLocation() + ", toLocation="
				+ this.getToLocation() + ", points=" + this.points + ", distance=" + this.distance + "]";
	}
	...
	public int compareTo(MissionCard o) {
		...
		if (pointCompare == 0) {
			for (int i = 0, max = Math.min(this.locations.size(), o.locations.size()); i < max; i++) {
				int compare = this.locations.get(i)
						.compareTo(o.locations.get(i));
